...
const Dashboard = () => {
  ...
  const fetchKyc = async () => {
    try{
      const response = await axios.get("https://ecommerce-backend-ochre-phi.vercel.app/api/v1/kyc/KycList" , Auth)
      const KycStatus = response.data?.data?.[0]?.kycStatus
      localStorage.setItem("Kyc-Status" , KycStatus)
    }catch{}
  }
  ...
  console.log(localStorage.getItem("Kyc-Status"))
