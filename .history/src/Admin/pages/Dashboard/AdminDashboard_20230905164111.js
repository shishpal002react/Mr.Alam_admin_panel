...
const AdminDashboard = () => {
  ...
  const fetchHandler = async () => {
    setLoading(true)
    try{
      const response = await axios.get("https://ecommerce-backend-ochre-phi.vercel.app/api/v1/admin/dashboard")
  ...
  const card = [
    ...
    },
    {
      ...
      icon: (
        <i className="fa-solid fa-clipboard text-2xl text-[#023b5b]"></i>
      ),
      bg: "#023b5b",
      link: "/admin/category",
    },
    {
      ...
      icon: (
        <i className="fa-solid fa-folder text-2xl text-[#72909e]"></i>
      ),
      bg: "#72909e",
    ...
    },
    {
      ...
      number: count?.user,
      icon: (
        <i className="fa-solid fa-user text-2xl text-[#2f6967]"></i>
      ),
      bg: "#2f6967",
      link: "/user-list",
    },
    {
      ...
      number: count?.vendor,
      icon: (
        <i className="fa-solid fa-circle-user text-2xl text-[#04649b]"></i>
      ),
      bg: "#04649b",
      link: "/vendor-list",
    },
  ...
  return (
    <>
    {
      loading ?  <Spinner animation="border" role="status" className="loadingSpin" /> :   <section className="HomeGrid">
        {card.map((card, index) => {
          return (
            <div
