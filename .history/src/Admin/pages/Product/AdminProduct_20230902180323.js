  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const { data } = await axios.get(
        `https://ecommerce-backend-ochre-phi.vercel.app/api/v1/admin/Product/list?page=${page}&limit=${limit}&search=${search}&toDate=${FinalToDate}&fromDate=${FinalFromDate}&categoryId=${categoryId}&subcategoryId=${subcategoryId}`,
        Auth
      );
                    ...
                    {data?.map((i, index) => (
                      <tr key={index}>
                        ...
                        <td>
                          {i.varient === true ? (
                            <Link
                              to={`/admin/product-variant/${i._id}/${i.productName}`}
                            >
                              <button className="md:py-2 px-3 md:px-4 py-1 rounded-sm bg-[#0c0c0c] text-white tracking-wider">
                                View
                              </button>
                        ...
                        <td>
                          <span className="flexCont">
                            <Link to={`/admin/edit-product/${i._id}`}>
